  ...
  def save_to_dependencies(group)
    group.periods.each do |group_period|
      group.users.each do |group_student|
        ...
        attendances = Attendance.where(period_id: group_period.id, users_id: group_student.id)
        homeworks = Homework.where(period_id: group_period.id, users_id: group_student.id)

        if attendances.blank?
          attendance = Attendance.new(period_id: group_period.id, users_id: group_student.id)
          attendance.save
        end
